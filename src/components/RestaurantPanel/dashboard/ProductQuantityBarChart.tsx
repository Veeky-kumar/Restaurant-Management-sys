...
const ProductQuantityBarChart: React.FC = () => {
  type TimePeriod = 'Yearly' | 'Monthly' | 'Weekly' | 'Daily';
  type ProductFilter = 'quantity' | 'amount' | 'profit';

  const [timePeriod, setTimePeriod] = useState<TimePeriod>('Weekly');
  const [productFilter, setProductFilter] = useState<ProductFilter>('quantity');
  ...
  const chartOptions: Options = {
    ...
    xAxis: {
      categories: chartData.length > 0 ? chartData.map(item => item.name) : ['Product 1', 'Product 2', 'Product 3', 'Product 4', 'Product 5', 'Product 6', 'Product 7'],
      labels: {
        style: {
          fontSize: '14px',
        },
    ...
    series: [
      {
        type: 'bar',
        name: 'Sales',
        data: chartData.length > 0 ? chartData.map(item => item.y) : [1, 2, 3, 4, 5, 6,7],
        pointWidth: 30,
  ...
  const handleProductFilterChange = (value: ProductFilter) => {
    ...
    <div className="col-sm-6 col-md-6 col-lg-6 col-xl-4 col-12">
      ...
      <div className="wraps_dashbords graph_wrapsproduts" style={{ padding: '12px' }}>
        ...
        <div className='grid grid-cols-4 sm:grid-cols-2 '>
          <div className="w-fit col-span-3 sm:col-span-1 sm:p-1 wrap_dropsdownq">
            <Dropdown>
          ...
          <div className="w-fit translate-x-3 sm:translate-x-0 sm:p-1 col-span-1 wrap_dropsdowns" id="productCard2_graphfilter" style={{ zIndex: 2 }}>
            <Dropdown style={{ marginBottom: '10px' }}>
              <Dropdown.Toggle className='' variant="secondary" id="time-period-dropdown">
                {timePeriod}
              </Dropdown.Toggle>
